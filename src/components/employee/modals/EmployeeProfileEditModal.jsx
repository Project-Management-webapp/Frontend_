...
const EmployeeProfileEditModal = ({ isOpen, onClose, userData, onSave, onShowToast }) => {
  ...
  useEffect(() => {
    ...
    const formattedData = {
      ...
      probationEndDate: userData.probationEndDate ? new Date(userData.probationEndDate).toLocaleDateString('en-CA') : '',
      confirmationDate: userData.confirmationDate ? new Date(userData.confirmationDate).toLocaleDateString('en-CA') : '',
    ...
    >
      ...
      >
        ...
        <div className="flex justify-end space-x-4 p-6 pt-4 border-t border-gray-700">
          <button type="button" onClick={onClose} disabled={saving} className="px-4 py-2 cursor-pointer bg-gray-600 rounded-md hover:bg-gray-700 transition-colors">
            Cancel
          </button>
